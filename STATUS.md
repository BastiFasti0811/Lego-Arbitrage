...
**Nächste konkrete Handlung:** [scrape_daily.py:85](backend/app/tasks/scrape_daily.py:85) fixen — `from datetime import UTC, datetime` und `datetime.now(UTC)` — plus Regressionstest, dann Push auf main (Auto-Deploy).

**Die 3 größten Risiken:**
1. **Stiller Datenausfall:** `offers`/`price_records` bekommen seit ~5 Monaten keine Writes; der Heartbeat-Watchdog misst nur „Task lief", nicht „Task hat Daten geschrieben" — er ist für genau diesen Fehlertyp blind.
2. **Backup-Kette unbelegt:** Cron nur manuell einzurichten, das Doku-Beispiel zeigt auf den **falschen Pfad** (`/opt/` statt `/srv/`), Restore nie getestet, Off-site per Default aus. Ob auf dem Server je ein Backup lief: unbekannt.
...
|---|---|---|---|---|
| Prod-Deploy-Pipeline (CI verify + SSH-Deploy) | [deploy.md](docs/deploy.md) 2026-06-29 | [deploy-production.yml](.github/workflows/deploy-production.yml): pytest, ruff, alembic check, Compose-/Caddy-Validierung, Deploy nach `/srv/lego-arbitrage`; Container liefen 11.08. | WIRKSAM* | Deploy-Weg für den Fix ist vorhanden und scharf |
...
| Heartbeat/Pipeline-Health (`task_heartbeats`, `/api/system/status`) | PR #5 „P0 trust & safety" 2026-06-29 | [heartbeat.py](backend/app/services/heartbeat.py), [health_check.py](backend/app/tasks/health_check.py); misst nur Task-Erfolg | WIRKSAM* | Blind für geschluckte Fehler → hat den Scrape-Ausfall 5 Monate nicht gemeldet |
...
| „Automated DB backups" | Commit 8436673 (2026-06-29) | Skripte gemergt ([backup-db.sh](scripts/backup-db.sh), CRLF-Fix 2026-06-30); Cron nur als manuelle Anleitung, Beispiel zeigt auf `/opt/lego-arbitrage` statt `/srv/` ([deploy.md:173](docs/deploy.md:173)); Restore-Verify explizit offen (deploy.md:180-182); `BACKUP_REMOTE` default aus | GEMERGT/NICHT-AUSGEROLLT | Server-Check nötig; Doku-Pfad korrigieren |
...
| Server-Laufzeit | `spm-prod-01` (178.104.97.121), `/srv/lego-arbitrage` | einziger Beleg: fremde Session 2026-08-11, `running(6)` | niedrig-mittel — SSH-Audit ausstehend |
